...
ax = [ax]
dr_data_path = os.path.join("data", "IBDR")
program_data = pd.read_csv(
    os.path.join(dr_data_path, "us_program_data.csv"), encoding="latin1"
)

# Group by state and count # of programs
state_program_counts = (
    program_data.groupby("state").size().reset_index(name="num_programs")
)
...
result_with_all_states = (
    result.set_index("state").reindex(combined_states).reset_index()
)
...
merged_data = us_states.merge(
    state_program_counts, left_on="STUSPS", right_on="state", how="left"
)
...
def makeColorColumn(data, column, vmin, vmax, cmap="viridis"):
    ...
    data["value_determined_color"] = data[column].apply(
        lambda x: mcolors.to_hex(cmap(norm(x))) if pd.notnull(x) else "#D3D3D3"
    )
    return data
...
def create_map(data, column, title, cmap="viridis"):
    ...
    alaska = visframe[visframe["STUSPS"] == "AK"].scale(
        xfact=0.50, yfact=0.50, origin=(0, 0)
    )
    alaska = alaska.translate(xoff=-470000, yoff=-3500000)
    alaska.plot(
        color=data[data["STUSPS"] == "AK"]["value_determined_color"].iloc[0],
        linewidth=1,
        ax=ax[0],
        edgecolor="black",
    )
    ...
    hawaii.plot(
        color=data[data["STUSPS"] == "HI"]["value_determined_color"].iloc[0],
        linewidth=1,
        ax=ax[0],
        edgecolor="black",
    )
...
iso_rto_durations = (
    data.groupby(["iso/rto"])[["min_dur", "max_dur"]].mean().reset_index()
)
...
iso_rto_durations["dur_height"] = (
    iso_rto_durations["max_dur"] - iso_rto_durations["min_dur"]
)
...
iso_rto_durations["iso/rto"] = pd.Categorical(
    iso_rto_durations["iso/rto"], categories=region_order, ordered=True
)
...
# Load the data
prices = pd.read_csv(
    os.path.join(dr_data_path, "on_off_peak_prices.csv"), encoding="latin1"
)
...
result = (
    prices_expanded.groupby("iso/rto")
    .agg(
...
summer = ax[1].bar(
    x, data["max_summer_price"], width, color="#E2ECF7", edgecolor="black"
)
