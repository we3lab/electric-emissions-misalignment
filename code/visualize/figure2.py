...
for i,r in enumerate(regions):
    ...
    if OVERLAY_POINTS:
        ...
        ax[0].step(xs, top, lw=overlay_params["lw"], color=overlay_params["color"], where=overlay_params["where"], ls=overlay_params["linestyle"], alpha=overlay_params["alpha"])
        ax[0].step(xs, bottom, lw=overlay_params["lw"], color=overlay_params["color"], where=overlay_params["where"], ls=overlay_params["linestyle"], alpha=overlay_params["alpha"])
        # connect the top and bottom on the left and right
        ax[0].vlines(xs[0], bottom[0], top[0], lw=overlay_params["lw"], color=overlay_params["color"], ls=overlay_params["linestyle"], alpha=overlay_params["alpha"])
        ax[0].vlines(xs[-1], bottom[-1], top[-1], lw=overlay_params["lw"], color=overlay_params["color"], ls=overlay_params["linestyle"], alpha=overlay_params["alpha"])

    # for each month 
    for m in range(12):
        month_aef = df_aef[df_aef['month'] == m+1]
        co2i_aef = month_aef['co2_eq_kg_per_MWh']
        ax[0].scatter(10*np.ones_like(co2i_aef) * (i+1) + month_mod[m], 
                    co2i_aef, 
    ...
    ax[0].set(ylabel='Carbon Intensity (kg CO$_2$-eq/MWh)',
...
for i,r in enumerate(regions):
    ...
    if OVERLAY_POINTS:
        ...
        ax[1].step(xs, top, lw=overlay_params["lw"], color=overlay_params["color"], where=overlay_params["where"], ls=overlay_params["linestyle"], alpha=overlay_params["alpha"])
        ax[1].step(xs, bottom, lw=overlay_params["lw"], color=overlay_params["color"], where=overlay_params["where"], ls=overlay_params["linestyle"], alpha=overlay_params["alpha"])
        # connect the top and bottom on the left and right
        ax[1].vlines(xs[0], bottom[0], top[0], lw=overlay_params["lw"], color=overlay_params["color"], ls=overlay_params["linestyle"], alpha=overlay_params["alpha"])
        ax[1].vlines(xs[-1], bottom[-1], top[-1], lw=overlay_params["lw"], color=overlay_params["color"], ls=overlay_params["linestyle"], alpha=overlay_params["alpha"])

                    
    for m in range(12):
        month = df_mef[(df_mef['month'] == m+1) & (df_mef['isorto'] == r)]
        co2i = month['co2_eq_kg_per_MWh']
        ax[1].scatter(10*np.ones_like(co2i) * (i+1) + month_mod[m], 
                    co2i, 
                    color=mpl.colors.rgb2hex(cmap(m)), 
                    # edgecolor='black',
                    marker='s',
    ...
    ax[1].set(ylabel='Carbon Intensity (kg CO$_2$-eq/MWh)',
            xticks=10*np.arange(1, len(regions)+1),
            xticklabels=regions,
            ylim=(0, 800),
            yticks=np.arange(0, 900, 100))
