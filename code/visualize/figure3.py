...
def calculate_charge_summary(df, datetime_range):
    ...
    charge_dict = get_charge_dict(effective_start_date, effective_end_date, df, resolution="15m")
    ...
    for charge_type in ["energy", "demand"]: 
        ...
        for tier in tiers:
            processed_keys = [key for key in relevant_keys if f"{tier}" == key.split("_")[-1]]
            ...
            means = charge_means[charge_type] = np.nanmean(aggregate_charge)
...
tariff_list = glob.glob("data/tariffs/processed_sheets/*.csv")

# define a placeholder dataframe to store the results
df_charges_summer = pd.DataFrame(columns=["total_monthly_cost", "total_average_cost_kwh", "energy_charge_ratio", "demand_charge_ratio", "mean_energy_charge", "mean_demand_charge", "max_energy_charge", "max_demand_charge", "energy_tiers", "demand_tiers"])
...
for tariff in tariff_list:
    ...
    try:
        ...
        df_charges_summer = pd.concat([df_charges_summer, pd.DataFrame(charge_summary, index=[tariff])], ignore_index=True)
    except:
        print("Failure for tariff:", tariff)
        pass
    try:
        charge_summary = calculate_charge_summary(df, winter_daterange)
        df_charges_winter = pd.concat([df_charges_winter, pd.DataFrame(charge_summary, index=[tariff])], ignore_index=True)
    except:
        print("Failure for tariff:", tariff)
        pass
...
ax[0,0].set(xlabel= "Mean energy charge\n($/kWh)", 
        ylabel="Peak Energy Charge Premium\n(max/min)",
        xlim=(0, 1.25),
        ylim=(0, 100),
)
ax_inset = ax[0,0].inset_axes([0.4, 0.4, 0.55, 0.55]) # x, y, width, height (fractions of parent axes)
ax_inset.scatter(df_charges_summer["mean_energy_charge"].values,
            df_charges_summer["energy_charge_ratio"],
            color="#A1645E",
            alpha=0.7,
            label="Summer",
...
ax[0,1].set(xlabel= "Mean energy charge\n($/kWh)", 
    ylabel="Peak Energy Charge Premium\n(max/min)",
    xlim=(0, 1.25),
    ylim=(0, 40),
)

ax_inset = ax[0,1].inset_axes([0.4, 0.4, 0.55, 0.55]) # x, y, width, height (fractions of parent axes)
ax_inset.scatter(df_charges_winter["mean_energy_charge"].values,
    df_charges_winter["energy_charge_ratio"],
    color="#5E9BA1",
    alpha=1,
    label="Winter",
...
df_charges_summer["demand_charge_ratio"] = df_charges_summer["demand_charge_ratio"].replace({0: -1})
...
ax[1,0].set(xlabel= "Max demand charge\n($/kW)", 
...
ax_inset = ax[1,0].inset_axes([0.4, 0.4, 0.55, 0.55]) # x, y, width, height (fractions of parent axes)
ax_inset.scatter(df_charges_summer["max_demand_charge"].values,
...
# replace all the demand charge ratios < 1 with -1
df_charges_winter["demand_charge_ratio"] = df_charges_winter["demand_charge_ratio"].replace({0: -1})

ax[1,1].scatter(df_charges_winter["max_demand_charge"].values, 
...
ax[1,1].set(xlabel= "Max demand charge\n($/kW)", 
        ylabel="Peak Demand Charge Premium\n(max/min)",
        xlim=(0, 100),
        ylim=(0, 100),
)
...
ax_inset = ax[1,1].inset_axes([0.4, 0.4, 0.55, 0.55]) # x, y, width, height (fractions of parent axes)
ax_inset.scatter(df_charges_winter["max_demand_charge"].values,
...
for i,r in enumerate(regions):
    ...
    f = os.path.join(lmp_path, f'{r}costs.csv')
    # get the lmp for the region
    # lmp = pd.read_csv(os.path.join(lmp_path, f'{r}costs.csv'))

# for i, f in enumerate(fnames_lmp):
    ...
    if "OTHER" in f:
        ...
        for m in range(12):
            ...
            ax[3,1].scatter(10*np.ones_like(price) * (i+1) + month_mod[m], 
                        price, 
...
ax[3,1].hlines(y=0, xmin=3, xmax=10*(len(fnames_lmp)+0.5)+2, color='black', ls="--", lw=2)

ax[3,1].set(ylabel='DAM Price\n($/MWh)',
...
metadata_path = os.path.join('data/tariffs/data/metadata_iso.csv')
metadata = pd.read_csv(metadata_path)
tariff_gpd = gpd.GeoDataFrame(geometry=gpd.points_from_xy(metadata.longitude, metadata.latitude), data=metadata)

combined_tariff_files = glob.glob(os.path.join(tariff_path,'combined/*.csv'))
...
tariff_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__vsc_ipynb_file__))), 'data', 'tariffs', 'timeseries', "combined")
timeseries_fps = glob.glob(tariff_path + '/*.csv')

metadata_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__vsc_ipynb_file__))), 'data', 'tariffs', 'data', "metadata_iso.csv")
metadata = pd.read_csv(metadata_path)
...
for t in timeseries_fps:
    ...
    if iso is not np.nan:
        for m in range(1,13):
            lmp = get_lmp_month(iso, enum_month[str(m)])
...
ax[2,1].scatter(peak_off_peak_win[:,0], peak_off_peak_win[:,1], alpha=0.5, color="#479DA3", lw=1, edgecolors='k',label="Winter Months (Nov-Apr)")
ax[2,1].scatter(peak_off_peak_sum[:,0], peak_off_peak_sum[:,1], alpha=0.5, color="#AB605B", lw=1, edgecolors='k',label="Summer Months (May-Oct)")
...
for label, axis in zip(labels, ax.flatten()):
    ...
    axis.text(
        ...
        1.0, 
        label, 
        transform=(axis.transAxes + ScaledTranslation(-42/72, 0, fig.dpi_scale_trans)),
        va='bottom',
        fontsize=10
    )
