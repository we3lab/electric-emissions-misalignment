...
for region in regions:
    ...
    regional_data['co2_eq_kg_per_MWh'] = regional_data['co2_eq_kg_per_MWh'].astype(float)
    ...
    for month in range(1,13):
        ...
        for f in file:
            if month_to_str[month] in f:
                ...
                # append
                regional_data = pd.concat([regional_data,pd.DataFrame(np.hstack([month_data.reshape(-1,1),hour_data.reshape(-1,1),co2i.reshape(-1,1)]),columns=['month','hour','co2_eq_kg_per_MWh'])],axis=0)

    # save the data
    regional_data = regional_data.reset_index(drop=True)
    regional_data.to_csv(os.path.join(basepath, "data", "MEFs", region + 'mef.csv'), index=False)
...
national_data.to_csv(os.path.join(basepath, "data", "MEFs", "average_mefs.csv"), index=False)