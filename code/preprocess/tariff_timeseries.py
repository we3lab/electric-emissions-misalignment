...
def calculate_energy_charge_timeseries(tariff, charge_dict, load_array, consumption_estimate):
...
def calculate_demand_charge_timeseries(tariff, charge_dict, load_array, consumption_estimate):
    result = np.zeros(len(load_array))
    # loop through energy charges
    for key, charge_array in charge_dict.items():
        utility, charge_type, name, eff_start, eff_end, limit_str = key.split("_")
...
def create_monthly_timeseries(tariff, month, load_kw=1000):
    ...
    result["Cost"] = (
        calculate_demand_charge_timeseries(tariff, charge_dict, flat_load, consumption_estimate) 
        + calculate_energy_charge_timeseries(tariff, charge_dict, flat_load, consumption_estimate)
    )
    return result
...
tariff_list = glob.glob(os.path.join(basepath, "data", "tariffs", "processed_sheets", "*.csv"))

num_month_to_str = { 1: "Jan", 2: "Feb", 3: "Mar", 4: "Apr", 5: "May", 6: "Jun", 7: "Jul", 8: "Aug", 9: "Sep", 10: "Oct", 11: "Nov", 12: "Dec" }
...
if combine_sheets:
    for key, files in tariff_dict.items():
        ...
        combined_df = combined_df.sort_values(by=['Month', 'Hour']).reset_index(drop=True)
        # save the combined dataframe to a csv file
        combined_df.to_csv(os.path.join(tariff_path, f'combined/tariff_{key}.csv'), index=False)