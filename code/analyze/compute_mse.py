...
for _iso in iso_regions:
    ...
    for _month in range(1, 13):
        ...
        if _iso in ["CAISO", "ISONE", "NYISO", "PJM"]:
            lmp_data = pd.read_csv(
                f"data/DAMs/{_iso}/{_iso}_DAM_2023_{month_map[_month]}_HOUR_RAW.csv",
                parse_dates=["Time"],
            )
            tmp = lmp_data["Time"].apply(
                lambda x: pd.to_datetime(x, utc=True)
                .tz_convert(ut.TIMEZONE_DICT["iso_rto_code"][_iso])
                .replace(tzinfo=None)
            )
            lmp_data["Time"] = tmp
        elif _iso == "SPP":
            ...
            lmp_no_time = pd.read_csv(
                f"data/DAMs/{_iso}/{_iso}_DAM_2023_HOUR.csv", parse_dates=["MARKET_DAY"]
            )
            lmp_data = []
            for timestamp in datetime_series:
                hour = timestamp.hour
        ...
        mef_hourly_profile = mef_data[
            (mef_data["month"] == _month) & (mef_data["isorto"] == _iso)
        ]
        ...
        mef_sample_data["co2_eq_kg_per_MWh"] = mef_sample_data.map(
            lambda row: mef_hourly_profile[(mef_hourly_profile["hour"] == row.hour)][
                "co2_eq_kg_per_MWh"
            ].values[0]
        )
        # center and normalize the MEF
        mef_sample_data["co2_eq_kg_per_MWh"] = (
            (mef_sample_data["co2_eq_kg_per_MWh"] - mef_sample_data["co2_eq_kg_per_MWh"]) 
            / mef_sample_data["co2_eq_kg_per_MWh"].max()
        )
        mef_list = []
        lmp_list = []
        ...
        for location in locations:
            ...
            try:
                lmp_list = lmp_list + lmp_sample["LMP"].tolist()
                mef_list = mef_list + mef_values.tolist()
            except (ValueError, TypeError) as ex:
                print("Error for location {}: ".format(location), ex)
        # center and normalize the LMP
        lmp_array = np.array(lmp_list)
        lmp_array = (lmp_array - lmp_array.mean()) / lmp_array.max()
        mse = mean_squared_error(np.array(mef_list), lmp_array)
        dam_mef_mse_dict[_iso].append(
            {
                "month": _month,
                "mse": mse,
            }
    ...
    mse_values = pd.DataFrame(dam_mef_mse_dict[_iso])
    mse_values.to_csv(f"data/MSE/{_iso}_mef_mse.csv", index=False)
