...
def get_sample_tariff_data(iso_monthly_tariffs, region, month, aef_values, iso_tariff_ids):
    corr_coefs = []
    ...
    # exclude nan values
    tariff_dfs = [df["Cost"].values for df in tariff_df_list if not df["Cost"].isna().all()]
    ...
    for tariff in tariff_dfs:
        tariff_data = pd.DataFrame({"DateTime": tariff_df_list[0]["DateTime"], "Cost": tariff})
...
metadata_path = os.path.join(basepath, "data", "tariffs", "metadata_iso.csv")
...
for region in regions:
    iso_tariff_label_dict[region] = metadata.loc[metadata['ISO'] == region, "label"].values
    print(region, len(iso_tariff_label_dict[region]))

iso_monthly_tariffs = {region: {month: [] for month in months} for region in regions if isinstance(region, str)}
...
for region in regions:
    ...
    for month in months:
        ...
        aef_data = pd.read_csv(f"data/AEFs/{region}/{region}_aef_historical_{month}_HOUR.csv")
        aef_values = aef_data["co2_eq_kg_per_MWh"].values
        corr_dict[region][month][month], corr_dict[region][month]["tariff_ids"] = get_sample_tariff_data(
            iso_monthly_tariffs, 
            region,
            month, 
            aef_values, 
